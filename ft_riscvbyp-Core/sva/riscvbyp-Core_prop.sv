// This property file was autogenerated by AutoSVA on 2024-05-05
...
wire [7:0] jal_ir_transid;
wire [7:0] jal_rf_transid;
wire jal_ir_rdy;
wire jal_ir_val;
wire jal_rf_rdy;
wire jal_rf_val;
wire [31:0] jal_ir_data;
wire [31:0] jal_rf_data;
...
wire [7:0] symb_jal_ir_transid;
am__symb_jal_ir_transid_stable: assume property($stable(symb_jal_ir_transid));
wire jal_rf_hsk = jal_rf_val && jal_rf_rdy;
wire jal_ir_hsk = jal_ir_val && jal_ir_rdy;
...
// Modeling incoming request for branch_ir_pc_trans
if (ASSERT_INPUTS) begin
	as__branch_ir_pc_trans_fairness: assert property (branch_pc_val |-> s_eventually(branch_pc_rdy));
end else begin
	am__branch_ir_pc_trans_fairness: assume property (branch_pc_val |-> s_eventually(branch_pc_rdy));
end

// Generate sampling signals and model
reg [3:0] branch_ir_pc_trans_transid_sampled;
wire branch_ir_pc_trans_transid_set = branch_ir_hsk && branch_ir_transid == symb_branch_ir_transid;
wire branch_ir_pc_trans_transid_response = branch_pc_hsk && branch_pc_transid == symb_branch_ir_transid;

always_ff @(posedge clk) begin
	if(reset) begin
		branch_ir_pc_trans_transid_sampled <= '0;
	end else if (branch_ir_pc_trans_transid_set || branch_ir_pc_trans_transid_response ) begin
		branch_ir_pc_trans_transid_sampled <= branch_ir_pc_trans_transid_sampled + branch_ir_pc_trans_transid_set - branch_ir_pc_trans_transid_response;
	end
end
co__branch_ir_pc_trans_transid_sampled: cover property (|branch_ir_pc_trans_transid_sampled);
if (ASSERT_INPUTS) begin
	as__branch_ir_pc_trans_transid_sample_no_overflow: assert property (branch_ir_pc_trans_transid_sampled != '1 || !branch_ir_pc_trans_transid_set);
end else begin
	am__branch_ir_pc_trans_transid_sample_no_overflow: assume property (branch_ir_pc_trans_transid_sampled != '1 || !branch_ir_pc_trans_transid_set);
end


// Assert that if valid eventually ready or dropped valid
as__branch_ir_pc_trans_transid_hsk_or_drop: assert property (branch_ir_val |-> s_eventually(!branch_ir_val || branch_ir_rdy));
// Assert that every request has a response and that every reponse has a request
as__branch_ir_pc_trans_transid_eventual_response: assert property (|branch_ir_pc_trans_transid_sampled |-> s_eventually(branch_pc_val && (branch_pc_transid == symb_branch_ir_transid) ));
as__branch_ir_pc_trans_transid_was_a_request: assert property (branch_ir_pc_trans_transid_response |-> branch_ir_pc_trans_transid_set || branch_ir_pc_trans_transid_sampled);


// Modeling data integrity for branch_ir_pc_trans_transid
reg [31:0] branch_ir_pc_trans_transid_data_model;
always_ff @(posedge clk) begin
	if(reset) begin
		branch_ir_pc_trans_transid_data_model <= '0;
	end else if (branch_ir_pc_trans_transid_set) begin
		branch_ir_pc_trans_transid_data_model <= branch_ir_data;
	end
end

as__branch_ir_pc_trans_transid_data_unique: assert property (|branch_ir_pc_trans_transid_sampled |-> !branch_ir_pc_trans_transid_set);
as__branch_ir_pc_trans_transid_data_integrity: assert property (|branch_ir_pc_trans_transid_sampled && branch_ir_pc_trans_transid_response && !jump_taken_after_branch_reg |-> (branch_pc_data == branch_ir_pc_trans_transid_data_model));
...
// Modeling incoming request for jal_ir_rf_trans
if (ASSERT_INPUTS) begin
	as__jal_ir_rf_trans_fairness: assert property (jal_rf_val |-> s_eventually(jal_rf_rdy));
end else begin
	am__jal_ir_rf_trans_fairness: assume property (jal_rf_val |-> s_eventually(jal_rf_rdy));
end

// Generate sampling signals and model
reg [3:0] jal_ir_rf_trans_transid_sampled;
wire jal_ir_rf_trans_transid_set = jal_ir_hsk && jal_ir_transid == symb_jal_ir_transid;
wire jal_ir_rf_trans_transid_response = jal_rf_hsk && jal_rf_transid == symb_jal_ir_transid;

always_ff @(posedge clk) begin
	if(reset) begin
		jal_ir_rf_trans_transid_sampled <= '0;
	end else if (jal_ir_rf_trans_transid_set || jal_ir_rf_trans_transid_response ) begin
		jal_ir_rf_trans_transid_sampled <= jal_ir_rf_trans_transid_sampled + jal_ir_rf_trans_transid_set - jal_ir_rf_trans_transid_response;
	end
end
co__jal_ir_rf_trans_transid_sampled: cover property (|jal_ir_rf_trans_transid_sampled);
if (ASSERT_INPUTS) begin
	as__jal_ir_rf_trans_transid_sample_no_overflow: assert property (jal_ir_rf_trans_transid_sampled != '1 || !jal_ir_rf_trans_transid_set);
end else begin
	am__jal_ir_rf_trans_transid_sample_no_overflow: assume property (jal_ir_rf_trans_transid_sampled != '1 || !jal_ir_rf_trans_transid_set);
end


// Assert that if valid eventually ready or dropped valid
as__jal_ir_rf_trans_transid_hsk_or_drop: assert property (jal_ir_val |-> s_eventually(!jal_ir_val || jal_ir_rdy));
// Assert that every request has a response and that every reponse has a request
as__jal_ir_rf_trans_transid_eventual_response: assert property (|jal_ir_rf_trans_transid_sampled |-> s_eventually(jal_rf_val && (jal_rf_transid == symb_jal_ir_transid) ));
as__jal_ir_rf_trans_transid_was_a_request: assert property (jal_ir_rf_trans_transid_response |-> jal_ir_rf_trans_transid_set || jal_ir_rf_trans_transid_sampled);


// Modeling data integrity for jal_ir_rf_trans_transid
reg [31:0] jal_ir_rf_trans_transid_data_model;
always_ff @(posedge clk) begin
	if(reset) begin
		jal_ir_rf_trans_transid_data_model <= '0;
	end else if (jal_ir_rf_trans_transid_set) begin
		jal_ir_rf_trans_transid_data_model <= jal_ir_data;
	end
end

as__jal_ir_rf_trans_transid_data_unique: assert property (|jal_ir_rf_trans_transid_sampled |-> !jal_ir_rf_trans_transid_set);
as__jal_ir_rf_trans_transid_data_integrity: assert property (|jal_ir_rf_trans_transid_sampled && jal_ir_rf_trans_transid_response |-> (jal_rf_data == jal_ir_rf_trans_transid_data_model));
...
assign jal_rf_data = jal_rf_data_reg;
assign jal_ir_val = ctrl.inst_val_Dhl && (type_jal_instr_Dhl || type_jalr_instr_Dhl);
assign load_dmem_transid = load_dmem_transid_reg;
assign rf_rdy = 1'b1;
assign ir_data = alu_output_reg;
assign jal_rf_rdy = 1'b1;
assign load_ir_rdy = !stall_Dhl_reg;
assign load_rf_transid = load_rf_transid_reg;
assign jal_rf_val = ctrl.inst_val_Whl && ctrl.rf_wen_Whl && (type_jal_instr_Whl || type_jalr_instr_Whl);
...
assign jal_rf_transid = jal_rf_transid_reg;
...
assign jal_ir_rdy = !stall_Dhl_reg;
...
`ifdef XPROP
	 ...
	 as__no_x_jal_rf_val: assert property(!$isunknown(jal_rf_val));
	 as__no_x_jal_rf_data: assert property(jal_rf_val |-> !$isunknown(jal_rf_data));
	 as__no_x_jal_rf_transid: assert property(jal_rf_val |-> !$isunknown(jal_rf_transid));
	 ...
	 as__no_x_jal_ir_val: assert property(!$isunknown(jal_ir_val));
	 as__no_x_jal_ir_data: assert property(jal_ir_val |-> !$isunknown(jal_ir_data));
	 as__no_x_jal_ir_transid: assert property(jal_ir_val |-> !$isunknown(jal_ir_transid));
...
(
	...
	.type_jal_instr(type_jal_instr_Dhl),
	.type_jalr_instr(type_jalr_instr_Dhl)
); 
...
(
	...
	.type_jal_instr(type_jal_instr_Whl),
...
always_comb begin
	...
	if (type_jalr_instr_Dhl) begin
		calculated_stored_pc_jal = dpath.pc_Dhl + 32'd4; 
		calculated_pc_jal = (i_immediate_12_bits + rs1_data) & 32'hFFFF_FFFE;
	end 
end 
...
always_ff @(posedge clk) begin
	if (reset) begin
		...
		jal_ir_transid_reg <= 8'b0; 
		jal_rf_transid_reg <= 8'b0; 
	end else begin
		...
		if (jal_ir_rdy && jal_ir_val) begin 
			jal_ir_transid_reg <= jal_ir_transid_reg + 8'b1; 
		end 

		if (jal_rf_rdy && jal_rf_val) begin 
			jal_rf_transid_reg <= jal_rf_transid_reg + 8'b1; 
		end 
...
always @(posedge clk) begin
	if (reset) begin
		...
		if (jal_rf_val) begin
			jal_rf_data_reg <= dpath.wb_mux_out_Whl;
		end
...
as_rs1_match: assert property ((type_alu_r_instr_Dhl || type_alu_i_instr_Dhl || type_load_instr_Dhl || type_store_instr_Dhl || type_branch_instr_Dhl || type_jalr_instr_Dhl) && ctrl.inst_val_Dhl |-> (rs1 == dpath.rf_raddr0_Dhl));
...
// JAL/JALR: Assert that when JAL or JALR decoded, there will be squash in the fetch stage
as__jal_squash_fetch: assert property (ctrl.inst_val_Dhl && (type_jal_instr_Dhl || type_jalr_instr_Dhl) |-> ctrl.squash_Fhl);

// JAL/JALR: Assert that when JAL or JALR decoded, the correct PC + immediate is done
as__jal_ir_pc_trans_transid_data_integrity: assert property (ctrl.inst_val_Dhl && (type_jal_instr_Dhl || type_jalr_instr_Dhl) |-> (calculated_pc_jal == dpath.imemreq_msg_addr));
...
// Assert if squashed in the current stage, a bubble will be propagated to the next stage
...
as__squash_bubble_next_stage_D_X: assert property ((ctrl.squash_Dhl && !ctrl.stall_Xhl) |-> ##1 ctrl.bubble_Xhl); 
endmodule