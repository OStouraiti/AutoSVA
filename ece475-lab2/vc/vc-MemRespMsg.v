...
// responses currently include nothing other than the t.
//
// Message Format:
//
//    1b     calc   p_data_sz
...
//  +------+------+-----------+
//
// The message t is parameterized by the number of bits in the data.
...
#(
  ...
  input [`VC_MEM_RESP_MSG_TYPE_SZ(p_data_sz)-1:0] t,
  ...
  assign bits[`VC_MEM_RESP_MSG_TYPE_FIELD(p_data_sz)] = t;
...
#(
  ...
  output [`VC_MEM_RESP_MSG_TYPE_SZ(p_data_sz)-1:0] t,
  output [`VC_MEM_RESP_MSG_LEN_SZ(p_data_sz)-1:0] len,
  output [`VC_MEM_RESP_MSG_DATA_SZ(p_data_sz)-1:0] data
);

  assign t = bits[`VC_MEM_RESP_MSG_TYPE_FIELD(p_data_sz)];
...
#(
  ...
  wire [`VC_MEM_RESP_MSG_TYPE_SZ(p_data_sz)-1:0] t;
  ...
  always @(*) begin
    ...
    if ( msg === {c_msg_sz{1'bx}} )
      ...
      case ( t )
        ...
        default : $sformat( full_str, "undefined t" );
      endcase
    end
