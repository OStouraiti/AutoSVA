...
reg [63:0] mulreq_trans_transid_data_model;
always_ff @(posedge clk) begin
	if(reset) begin
...
wire [63:0] unsigned_res = {32'b0, unsigned_a } * {32'b0, unsigned_b };
wire [63:0] result = mulreq_msg_a[31] ^ mulreq_msg_b[31] ? ~unsigned_res + 1'b1 : unsigned_res; 
assign mulreq_transid = 1'b0;
assign mulreq_data = result;
					/*(mulreq_msg_a[31] ^ mulreq_msg_b[31]) 
					? ~({32'b0, (mulreq_msg_a[31] ?  ~mulreq_msg_a + 1'b1 : mulreq_msg_a)} 
					* {32'b0, (mulreq_msg_b[31] ?  ~mulreq_msg_b + 1'b1 : mulreq_msg_b)}) + 1'b1 :

					({32'b0, (mulreq_msg_a[31] ?  ~mulreq_msg_a + 1'b1 : mulreq_msg_a)} 
					* {32'b0, (mulreq_msg_b[31] ?  ~mulreq_msg_b + 1'b1 : mulreq_msg_b)});*/
